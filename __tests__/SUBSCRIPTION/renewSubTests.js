...
describe('/:userId/subscription/renew', () => {
    ...
    it("Invalid parameter in the URL", async () => {
        try {
            await axios.patch(`http://localhost:8081/users/${invalidParam}/subscription/renew`, {
                subscriptionPlan: "AA"
            }, {
                headers: {
    ...
    it("Succesfully changed subscription plan", async () => {
        try {
            const response = await axios.patch(`http://localhost:8081/users/${userId}/subscription/renew`, {
                subscriptionPlan: "UHD"
            }, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
